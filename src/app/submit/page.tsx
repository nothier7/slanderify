    ...
    <main className="min-h-dvh grid place-items-center p-8">
      <form onSubmit={onSubmit} className="card p-6 space-y-3 w-full max-w-md">
        ...
        <label className="block space-y-1">
          ...
          <input
            className="h-10 px-3 rounded-lg bg-card border border-border/20 outline-none w-full"
            value={realName}
        ...
        <label className="block space-y-1">
          ...
          <input
            className="h-10 px-3 rounded-lg bg-card border border-border/20 outline-none w-full"
            value={slander}
            onChange={(e)=>setSlander(e.target.value)}
            maxLength={64}
            required
          />
        </label>
        <Button type="submit" disabled={pending}>{pending ? "Submitting…" : "Submit"}</Button>
        {submitted && <div className="text-sm text-muted">Thanks! Your slander was submitted.</div>}
      </form>
    </main>
  );
}
